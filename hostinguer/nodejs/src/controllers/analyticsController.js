...
exports.obterPaginasPorUsuario = asyncHandler(async (req, res) => {
  const { usuarioId } = req.params;
  const { dias = 90 } = req.query;
  const data = await analyticsService.obterPaginasPorUsuario(usuarioId, Number(dias));
  res.json({ data });
});

exports.obterHeatmapPorUsuarioPagina = asyncHandler(async (req, res) => {
  const { usuarioId } = req.params;
  const { pagina, dias = 90 } = req.query;

  if (!pagina) {
    throw new AppError(400, 'Página é obrigatória');
  }

  const data = await analyticsService.obterHeatmapPorUsuarioPagina(usuarioId, pagina, Number(dias));
  res.json({ data });
});
