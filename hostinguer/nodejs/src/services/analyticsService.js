...
exports.registrarEventos = async (eventos, usuarioId = null) => {
  ...
  const now = new Date();
  const sessoes = [...new Set(eventos.map((e) => e.sessao_id).filter(Boolean))];

  if (usuarioId && sessoes.length > 0) {
    await db.AnalyticsComportamento.update(
      { usuario_id: usuarioId },
      {
        where: {
          sessao_id: { [Op.in]: sessoes },
          usuario_id: null,
        },
      }
    );
  ...
  const registros = eventos.map((e) => ({
    ...
    usuario_id: usuarioId || e.usuario_id || null,
    criado_em: now,
  }));
...
exports.obterPaginasPorUsuario = async (usuarioId, dias = 90) => {
  const dataLimite = new Date();
  dataLimite.setDate(dataLimite.getDate() - dias);

  const [usuario, paginas] = await Promise.all([
    db.Usuario.findByPk(usuarioId, {
      attributes: ['id', 'nome', 'email', 'tipo_usuario'],
      raw: true,
    }),
    db.AnalyticsComportamento.findAll({
      attributes: [
        'pagina',
        [fn('COUNT', col('id')), 'total_visualizacoes'],
        [fn('SUM', col('duracao_ms')), 'tempo_total_ms'],
        [fn('AVG', col('duracao_ms')), 'tempo_medio_ms'],
        [fn('MAX', col('created_at')), 'ultimo_acesso'],
      ],
      where: {
        usuario_id: usuarioId,
        tipo: 'page_view',
      ...
      group: ['pagina'],
      order: [[fn('SUM', col('duracao_ms')), 'DESC']],
      raw: true,
    }),
  ]);
  ...
  return {
    ...
    paginas: paginas.map((p) => ({
      pagina: p.pagina,
      total_visualizacoes: Number(p.total_visualizacoes),
      tempo_total_segundos: Math.round(Number(p.tempo_total_ms || 0) / 1000),
      tempo_medio_segundos: Math.round(Number(p.tempo_medio_ms || 0) / 1000),
      ultimo_acesso: p.ultimo_acesso,
    })),
  };
};

exports.obterHeatmapPorUsuarioPagina = async (usuarioId, pagina, dias = 90) => {
  const dataLimite = new Date();
  dataLimite.setDate(dataLimite.getDate() - dias);
  ...
  const [resumoRaw, pontos, elementos] = await Promise.all([
    db.AnalyticsComportamento.findAll({
      attributes: [
        [fn('COUNT', col('id')), 'total_eventos'],
        [fn('SUM', literal("CASE WHEN tipo = 'page_view' THEN 1 ELSE 0 END")), 'visualizacoes'],
        [fn('SUM', literal("CASE WHEN tipo = 'click' THEN 1 ELSE 0 END")), 'cliques'],
        [fn('SUM', literal("CASE WHEN tipo = 'hover' THEN 1 ELSE 0 END")), 'hovers'],
        [fn('SUM', col('duracao_ms')), 'tempo_total_ms'],
        [fn('AVG', col('duracao_ms')), 'tempo_medio_ms'],
        [fn('MAX', col('created_at')), 'ultimo_acesso'],
      ],
    ...
    db.AnalyticsComportamento.findAll({
      ...
      group: ['tipo', 'coordenada_x', 'coordenada_y', 'largura_tela', 'altura_tela', 'elemento'],
      order: [[fn('COUNT', col('id')), 'DESC']],
      limit: 500,
      raw: true,
    }),
    db.AnalyticsComportamento.findAll({
      attributes: ['tipo', 'elemento', [fn('COUNT', col('id')), 'total']],
      where: {
        ...baseWhere,
        [Op.or]: [{ tipo: 'click' }, { tipo: 'hover' }],
        elemento: { [Op.ne]: null },
  ...
  const resumo = resumoRaw[0] || {};

  return {
    pagina,
    resumo: {
      total_eventos: Number(resumo.total_eventos || 0),
      visualizacoes: Number(resumo.visualizacoes || 0),
      cliques: Number(resumo.cliques || 0),
      hovers: Number(resumo.hovers || 0),
      tempo_total_segundos: Math.round(Number(resumo.tempo_total_ms || 0) / 1000),
      tempo_medio_segundos: Math.round(Number(resumo.tempo_medio_ms || 0) / 1000),
      ultimo_acesso: resumo.ultimo_acesso || null,
    },
    pontos: pontos.map((p) => ({
      tipo: p.tipo,
      coordenada_x: Number(p.coordenada_x),
      coordenada_y: Number(p.coordenada_y),
      largura_tela: p.largura_tela ? Number(p.largura_tela) : null,
      altura_tela: p.altura_tela ? Number(p.altura_tela) : null,
      elemento: p.elemento,
...
exports.obterUsuariosPorMes = async (dias = 90) => {
  ...
  try {
    ...
    registros = await db.sequelize.query(
      ...
      GROUP BY ${mesExpr}, ac.usuario_id, u.nome, u.email
