...
function KpiConfigModal({ isOpen, onClose, config, onConfigUpdated, type = 'faturamento' }) {
  ...
  const handleSave = async () => {
    ...
    if (isConversao && parseFloat(conversaoBaixa) >= parseFloat(conversaoAlta)) {
      setError('Conversão baixa deve ser menor que conversão alta');
      return;
    }
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <h2>{isRecompra ? 'Configurar Recompra' : isTicket ? 'Configurar Ticket Médio' : isVisitante ? 'Configurar Visitantes' : isConversao ? 'Configurar Conversão' : 'Configurar Faturamento'}</h2>
          ...
          {isRecompra ? (
            <>
              ...
              <p className={styles.info}>
                O card exibirá imagens diferentes com base nesta configuração:
                <br />
                Abaixo de {parseFloat(recompraBaixa || 0).toFixed(2)}%: Recompra Baixa
                <br />
                Entre {parseFloat(recompraBaixa || 0).toFixed(2)}% e {parseFloat(recompraAlta || 0).toFixed(2)}%: Recompra Média
                <br />
            ...
            <>
              ...
              <p className={styles.info}>
                ...
                <br />
                Entre R$ {parseFloat(ticketBaixo || 0).toFixed(2)} e R$ {parseFloat(ticketAlto || 0).toFixed(2)}: Ticket Médio
                <br />
            ...
            <>
              ...
              <p className={styles.info}>
                ...
                <br />
                Entre {parseFloat(visitanteBaixo || 0).toFixed(0)} e {parseFloat(visitanteAlto || 0).toFixed(0)} visitantes: Visitante Médio
                <br />
            ...
            <>
              <div className={styles.fieldGroup}>
                <label htmlFor="conversao-baixa">Conversão Baixa (%)</label>
                <input
                  id="conversao-baixa"
                  type="number"
                  step="0.01"
              ...
              <div className={styles.fieldGroup}>
                <label htmlFor="conversao-alta">Conversão Alta (%)</label>
                <input
                  id="conversao-alta"
                  type="number"
                  step="0.01"
              ...
              <p className={styles.info}>
                O card exibirá imagens diferentes com base nesta configuração:
                <br />
                Abaixo de {parseFloat(conversaoBaixa || 0).toFixed(2)}%: Conversão Baixa
                <br />
                Entre {parseFloat(conversaoBaixa || 0).toFixed(2)}% e {parseFloat(conversaoAlta || 0).toFixed(2)}%: Conversão Média
                <br />
                Acima de {parseFloat(conversaoAlta || 0).toFixed(2)}%: Conversão Alta
              </p>
            </>
          ) : (
            <>
              ...
              <p className={styles.info}>
                O card exibirá cores/imagens diferentes com base nesta configuração:
                <br />
                Abaixo de R$ {parseFloat(faturamentoBaixo || 0).toFixed(2)}: Faturamento Baixo
                <br />
                Entre R$ {parseFloat(faturamentoBaixo || 0).toFixed(2)} e R$ {parseFloat(faturamentoAlto || 0).toFixed(2)}: Faturamento Médio
                <br />
