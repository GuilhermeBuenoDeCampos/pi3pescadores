...
const etapaLabels = {
  ...
  enviado_concluido: 'Enviado -> concluído',
};
...
function LeadtimeDetalhado() {
  ...
  const tableRows = registros.map((item) => {
    ...
    return {
      ...
      cliente: usuario.nome || usuario.email || 'Cliente não identificado',
      ...
      <section className={styles.mainArea}>
        <header className={styles.header}>
          ...
          <div>
            ...
            <p>Tempo médio entre entrada, carrinho, confirmação, preparo, envio e conclusão.</p>
          </div>
                ...
                {tableRows.length === 0 && (
                  <tr>
                    <td colSpan="9" className={styles.emptyCell}>
                      {loading ? 'Carregando registros...' : 'Nenhum lead time encontrado no período.'}
                    </td>
                  </tr>
                )}
                {tableRows.map((row) => (
